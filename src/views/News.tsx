...
const news = [
  ...
  },
  {
    ...
    summary: "BOCRA partners with local media to educate Batswana on their rights when dealing with telecom service providers.",
    date: "2026-01-20",
    category: "Announcement",
    featured: false,
  },
...
export default function News() {
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-7xl mx-auto px-4 md:px-6 py-16 space-y-12">
      ...
      {/* Featured */}
      {featured && (
        <motion.div variants={fadeUp} className="neu-card overflow-hidden hover-lift cursor-pointer">
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {rest.map((item) => (
          <div key={item.title} className="glass-panel overflow-hidden hover-lift cursor-pointer group">
            {item.image && (
